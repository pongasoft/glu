...
def main():
    ...
    parser.add_option("-u", "--user", dest="user",
                      ...
                      help="GLU user to use for authentication, defaults to " +
                      user)

    parser.add_option("-x", "--xpassword", dest="password",
                      action="store",
                      help="Password. Warning password will appear in clear in"
                      " ps output. Use only for testing.")
    ...
    parser.add_option("-t", "--allTags", dest="allTags",
                      action="store",
                      help="Shortcut for querying by tags (all tags must be"
                      " present): frontend;backend")

    parser.add_option("-T", "--anyTag", dest="anyTag",
                      action="store",
                      help="Shortcut for querying by tags (any of the tags"
                      " need to be present): frontend;backend")
    ...
    parser.add_option("-p", "--parallel", dest="parallel",
                      action="store_true", default=False,
                      help="Perform action on all instances in parallel."
                      " Default is serial.")

    parser.add_option("-n", "--dryrun", dest="dryrun",
                      action="store_true", default=False,
                      help="Do a dry run of your plan. No changes will be"
                      " made. Default is false.")

    # options for status action
    parser.add_option("-s", "--systemFilter", dest="filter",
                      action="store",
                      help="Filter in DSL sytax for filtering the model."
    ...
    parser.add_option("-S", "--systemFilterFile", dest="filterFile",
                      action="store",
                      help="Filter file with filters in DSL sytax for"
                      " filtering the model. Applicable only with 'status'"
                      " command. See 'Filter Syntax' section here:"
                      " https://github.com/linkedin/glu/wiki/Console ")
    ...
    if action in ("start", "stop", "bounce", "deploy", "undeploy", "redeploy"):
        if filter is None:
            filter = client.generateSystemFilter(options.agent,
                options.instance, options.allTags, options.anyTag)
