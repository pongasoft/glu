...
#
"""Glu Console command line interface."""
...
# Add the console-cli lib directory to the Python import path.
bin_dir = os.path.abspath((os.path.dirname(sys.argv[0])))
lib_dir = os.path.join(os.path.dirname(bin_dir), 'lib', 'python')
site.addsitedir(lib_dir)
site.addsitedir(os.path.join(lib_dir, 'site-packages'))
...
def main():
    """Main, most of it is command line handling."""
    ...
    parser = OptionParser(
        usage=usage, version='@console.name@: @console.version@')
    ...
    parser.add_option(
        '-t', '--allTags', dest='allTags', action='store',
        help='Shortcut for querying by tags (all tags must be present):'
            ' frontend;backend'
    )
    parser.add_option(
        '-T', '--anyTag', dest='anyTag', action='store',
    ...
    parser.add_option(
        '-M', '--modelFile', dest='modelFile', action='store',
    ...
    parser.add_option(
        '-b', '--beautify', dest='beautify', action='store_true', default=False,
    ...
    if len(exclusive_options.keys()) > 1:
        parser.error('Only one of: agent, allTags, anyTag, instance, filter or'
            'all may be used.')
    ...
    if options.filterFile:
        with open(options.filterFile, 'r') as fileter_file:
            system_filter = fileter_file.read().strip()
    ...
    if options.model or options.modelFile:
        model_loaded = client.loadModel(
            modelUrl=options.model, modelFile=options.modelFile)
        print model_loaded

    if action in ('start', 'stop', 'bounce', 'deploy', 'undeploy', 'redeploy'):
        if system_filter is None:
            system_filter = client.generateSystemFilter(
                options.agent, options.instance,
                options.allTags, options.anyTag
            )
